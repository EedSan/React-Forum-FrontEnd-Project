export const createLoadingSelector = (requestNames, defaultLoading = true) => (state) => {
    const entries = Object.entries(state.loading).filter(([key]) => requestNames.includes(key));
...
export const createErrorSelector = (requestNames) => (state) => {
    const entries = Object.entries(state.error).filter(([key]) => requestNames.includes(key));
    if (entries[0]) {
...
export const createLoadingAndErrorSelector = (requestNames, defaultLoading = true) => {
