...
import {commentsSelector} from '../selector/selectors';

export const setComments = (comments) => ({type: 'SET_COMMENTS', comments,});
export const updateComment = (id, updates) => ({type: 'UPDATE_COMMENT', id, updates,});
...
export const submitComment = (commentDetails) => async (dispatch, getState) => {
    try {
        ...
        const response = await axios.post('/comments', {body, post_id, parent_comment_id,});
        ...
        dispatch({type: 'SUBMIT_COMMENT_FAILURE', message: e.message, response: e.response,});
    }
};
