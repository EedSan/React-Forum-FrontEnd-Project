...
export const editPost = (id, updates) => ({type: 'EDIT_POST', id, updates,});
...
export const submitPost = (postDetails) => async (dispatch) => {
    ...
    try {
        ...
        const response = await axios.post('/posts', {type, title, body, subreddit,});
        dispatch({type: 'SUBMIT_POST_SUCCESS'});
        return response.data;
    } catch (e) {
        dispatch({type: 'SUBMIT_POST_FAILURE', message: e.message, responses: e.response,});
    }
};
