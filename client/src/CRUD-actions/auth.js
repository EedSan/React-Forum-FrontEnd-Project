...
import {commentsSelector, postListSelector} from '../selector/selectors';
...
export const startLogin = (username, password) => async (dispatch) => {
    try {
        ...
        dispatch({type: 'LOGIN_FAILURE', message: e.message, response: e.response,});
    }
};
...
export const startLogout = () => async (dispatch, getState) => {
    try {
        ...
        dispatch({
            type: 'LOGOUT_FAILURE', message: e.message, response: e.response,
        });
    }
};
...
export const startRegister = (username, password) => async (dispatch) => {
    try {
        ...
        dispatch({type: 'REGISTER_FAILURE', message: e.message, response: e.response,});
    }
};
