...
import {Alert, AlertIcon, Box, Button, FormControl, Input, Stack,} from '@chakra-ui/react';
import {createLoadingAndErrorSelector} from '../../selector/selectors';
import {startLogin} from '../../CRUD-actions/auth';
...
class LoginPage extends React.Component {
    ...
    render() {
        ...
        return (
            <Box w={300} m="auto">
                {requireAuth && (<Alert status="warning" mb={2}><AlertIcon/>{requireAuth}</Alert>)}
                ...
                <form onSubmit={this.handleSubmit}>
                    <Stack spacing={3}>
                        ...
                        <Button type="submit" isLoading={isLoading || null}>Login</Button>
                    </Stack>
                </form>
            </Box>
        );
...
const {loadingSelector, errorSelector} = createLoadingAndErrorSelector(['LOGIN'], false);

const mapStateToProps = (state) => ({isLoading: loadingSelector(state), error: errorSelector(state),});

const mapDispatchToProps = (dispatch) => ({startLogin: (username, password) => dispatch(startLogin(username, password)),});
