...
const updateTableRow = async (table, id, allowedUpdates, updatesObj) => {
    ...
    if (!validUpdate) {
        const fieldsString = allowedUpdates.length > 1 ? allowedUpdates.slice(0, allowedUpdates.length - 1).join(', ') + ` and ${allowedUpdates[allowedUpdates.length - 1]}` : allowedUpdates[0]
        throw new Error(`Only the ${fieldsString} ${allowedUpdates.length === 1 ? 'field' : 'fields'} can be updated`)
    }

    const setFieldsClause = updateFields.map((updateField, index) => `${updateField} = $${index + 1}`).join(', ')
